...
app.use(express.json({
  type: () => true, // parse body on every request regardless of Content-Type header,
                     // so req.rawBody is always populated (Zenoti doesn't reliably
                     // send Content-Type: application/json on its webhook POSTs)
  verify: (req, res, buf) => {
    req.rawBody = buf;
  }
}));
...
// Auth: /login is public (no key needed - it's how a human gets a token).
// User creation (POST /users) is protected inside authRoutes.js itself.
app.use('/api/auth', authRoutes);

// Dashboard: requires a real logged-in human (JWT from /api/auth/login),
// not the internal API key.
